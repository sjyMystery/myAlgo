...
class InstrumentSubplot(Subplot):
    ...
    def on_bars(self, bars):
        super(InstrumentSubplot, self).on_bars(bars)
    ...
    def onOrderEvent(self, broker_, orderEvent):
        ...
        if self.__plotBuySell and orderEvent.type in (
        State.PARTIALLY_FILLED, State.FILLED) and order.instrument == self.__instrument:
            action = order.action
            execInfo = orderEvent.info
            if action in [Action.BUY, Action.BUY_TO_COVER]:
                self.getSeries("Buy", BuyMarker).addValue(execInfo.datetime, execInfo.price)
            elif action in [Action.SELL, Action.SELL_SHORT]:
                self.getSeries("Sell", SellMarker).addValue(execInfo.datetime, execInfo.price)
...
class StrategyPlotter(object):
    ...
    def __init__(self, strat, plotAllInstruments=True, plotBuySell=True, plotPortfolio=True):
        ...
        strat.bars_processed_events.subscribe(self.__on_barsProcessed)
        strat.broker.order_events.subscribe(self.__onOrderEvent)
    ...
    def __on_barsProcessed(self, strat, bars):
        ...
        if self.__portfolioSubplot:
            self.__portfolioSubplot.getSeries("Portfolio").addValue(dateTime, strat.broker.equity)
            # This is in case additional dataseries were added to the portfolio subplot.
            self.__portfolioSubplot.on_bars(bars)
