...
class SMAEventWindow(indicator.EventWindow):
...
class SMA(indicator.EventBasedFilter):
...
class EMAEventWindow(indicator.EventWindow):
    def __init__(self, period):
        assert (period > 1)
    ...
    def onNewValue(self, dateTime, value):
        ...
        # Formula from http://stockcharts.com/school/doku.php?id=chart_school:indicator_indicators:moving_averages
        if value is not None and self.windowFull():
            if self.__value is None:
                self.__value = self.getValues().mean()
            else:
...
class EMA(indicator.EventBasedFilter):
...
class WMAEventWindow(indicator.EventWindow):
...
class WMA(indicator.EventBasedFilter):
