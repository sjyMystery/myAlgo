...
class StdDevEventWindow(indicator.EventWindow):
...
class StdDev(indicator.EventBasedFilter):
...
class StdSpeedEventWindow(indicator.EventWindow):
    def __init__(self, period, ddof):
        assert (period > 0)
        super(StdSpeedEventWindow, self).__init__(period)
        self.__ddof = ddof
    ...
    def getValue(self):
        ret = None
        if self.windowFull():
            ret = self.getValues().std(ddof=self.__ddof) / self.__period
        return ret


class StdSpeed(indicator.EventBasedFilter):
    """Standard deviation filter.
    ...
    def __init__(self, dataSeries, period, ddof=0, maxLen=None):
        super(StdSpeed, self).__init__(dataSeries, StdSpeedEventWindow(period, ddof), maxLen)


class ZScoreEventWindow(indicator.EventWindow):
    def __init__(self, period, ddof):
        assert (period > 1)
...
class ZScore(indicator.EventBasedFilter):
    """Z-Score filter.
