...
class HighLowEventWindow(indicator.EventWindow):
...
class Low(indicator.EventBasedFilter):
...
class RangeEventWindow(indicator.EventWindow):
    def __init__(self, windowSize, ):
        super(RangeEventWindow, self).__init__(windowSize)
    ...
    def getValue(self):
        ...
        if self.windowFull():
            ...
            """
                表示这个周期内的波动范围
            """
            ret = (values.max() - values.min()) / values[0]
        return ret


class Range(indicator.EventBasedFilter):

    def __init__(self, dataSeries, period, maxLen=None):
        super(Range, self).__init__(dataSeries, RangeEventWindow(period), maxLen)
